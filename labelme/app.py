...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.fileFilter = QtWidgets.QComboBox()
        self.fileFilter.addItem("_All_")
        self.fileFilter.currentTextChanged.connect(self.categorySelectChanged)
    ...
    def importDirImages(self, dirpath, pattern=None, load=True):
        ...
        self.filename = None
        self.label_path_mapping = []
        self.fileListWidget.clear()
        for filename in self.scanAllImages(dirpath):
            if pattern and pattern not in filename:
                continue
            label_file = osp.splitext(filename)[0] + ".json"
            self.label_path_mapping.extend(
                utils.label_to_path(label_file, osp.splitext(filename)[-1])
            )
            if self.output_dir:
                label_file_without_path = osp.basename(label_file)
                label_file = osp.join(self.output_dir, label_file_without_path)
        ...
        # list categories in QComboBox
        self.label_path_mapping = pd.DataFrame(self.label_path_mapping)
        unique_labels = self.label_path_mapping["label"].unique()
        unique_labels.sort()
        for label in unique_labels:
            self.fileFilter.addItem(label)
        self.openNextImg(load=load)

    def categorySelectChanged(self, category):
        if category == "_ALL_":
            return
        self.fileListWidget.clear()
        filenames = self.label_path_mapping[
            self.label_path_mapping["label"] == category
        ]["filename"].unique()
